...
function ajax_tipo_gd_interno(ve_control){
	ajax = nuevoAjax();
  	ajax.open("GET", "../guia_despacho/ajax_tipo_gd_interno.php?ve_cod_tipo_guia_despacho="+ve_control.value, false);
    ajax.send(null);
...
function request_arriendo(){
      	var url = "../guia_despacho_arriendo/request.php";
		$.showModalDialog({
			 url: url,
			 dialogArguments: '',
			 height: 360,
			 ...
			 onClose: function(){ 
				...
				if(respuesta == 'NO_EXISTE'){
					...
					return false;
			    }else if(respuesta == 'DESPACHADO'){
			    	alert('No existen Modificaciones de Arriendo de tipo "Agregar" pendientes de despacho')
			     	return false;
			    }
