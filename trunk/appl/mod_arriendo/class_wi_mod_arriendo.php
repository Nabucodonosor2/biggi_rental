...
class wi_mod_arriendo extends w_cot_nv {
	...
	const K_EMITIDA = 1;
	const K_AUTORIZA_APROBADA = '998005';
	...
	function load_record(){
		...
		if ($cod_estado_mod_arriendo==self::K_MOD_ARRIENDO_APROBADO) {
			...
			$priv = $this->get_privilegio_opcion_usuario(self::K_AUTORIZA_APROBADA, $this->cod_usuario);
			if ($priv == 'E'){
				$this->b_no_save_visible = true;
				$this->b_modify_visible = true;
				$this->b_save_visible = true;
				
				$this->dws['dw_mod_arriendo']->set_entrable('OBS', false);
				$this->dws['dw_mod_arriendo']->set_entrable('PORC_IVA', false);
				$this->dws['dw_mod_arriendo']->set_entrable('REFERENCIA', false);
				$this->dws['dw_item_mod_arriendo']->set_entrable_dw(false);
				
				$sql = "SELECT COD_ESTADO_MOD_ARRIENDO 
							  ,NOM_ESTADO_MOD_ARRIENDO 
						FROM ESTADO_MOD_ARRIENDO
						WHERE COD_ESTADO_MOD_ARRIENDO IN (".self::K_MOD_ARRIENDO_APROBADO.", ".self::K_MOD_ARRIENDO_ANULADO.")
						ORDER BY COD_ESTADO_MOD_ARRIENDO";
	  
				$this->dws['dw_mod_arriendo']->controls['COD_ESTADO_MOD_ARRIENDO']->set_sql($sql);
				$this->dws['dw_mod_arriendo']->controls['COD_ESTADO_MOD_ARRIENDO']->retrieve();
			}else{
	...
	function validate_record(){
		if(!$this->is_new_record()){
		  	$cod_estado_mod_arr = $this->dws['dw_mod_arriendo']->get_item(0, 'COD_ESTADO_MOD_ARRIENDO');
			$tipo_mod_arriendo = $this->dws['dw_mod_arriendo']->get_item(0, 'TIPO_MOD_ARRIENDO');
			$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
			
			if($tipo_mod_arriendo == 'ELIMINAR' && $cod_estado_mod_arr == 3){
				$sql = "SELECT COD_ESTADO_GUIA_RECEPCION
						FROM GUIA_RECEPCION
						WHERE TIPO_DOC = 'MOD_ARRIENDO'
						AND COD_DOC = ".$this->get_key()."
						AND COD_ESTADO_GUIA_RECEPCION <> 3";
				$result = $db->build_results($sql);
				
				if(count($result) > 0)
					return 'No se puede anular, tiene guia de recepción arriendo en estado Ingresada o Impresa.';
					
			}else if($tipo_mod_arriendo == 'AGREGAR' && $cod_estado_mod_arr == 3){
				$sql = "SELECT COD_GUIA_DESPACHO 
						FROM GUIA_DESPACHO
						WHERE COD_TIPO_GUIA_DESPACHO = 5 -- MOD ARRIENDO
						AND COD_DOC = ".$this->get_key();
				$result = $db->build_results($sql);
				
				if(count($result) > 0)
					return 'No se puede anular, tiene Guia de Despacho asociada a está modificación de contrato.';
			}
		}
	}
