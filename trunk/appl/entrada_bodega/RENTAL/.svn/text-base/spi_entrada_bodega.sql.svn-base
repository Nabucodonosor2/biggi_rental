...
BEGIN
	...
	if (@vl_tipo_doc = 'ORDEN_COMPRA')begin
	...
	else if (@vl_tipo_doc = 'GUIA_DESPACHO')begin
		SELECT @vl_nro_doc = NRO_GUIA_DESPACHO
			  ,@vl_fecha_doc = CONVERT(VARCHAR, FECHA_GUIA_DESPACHO, 103)
		FROM GUIA_DESPACHO
		WHERE COD_GUIA_DESPACHO = @vl_cod_doc
		...
		SELECT @vl_nro_doc = COD_GUIA_RECEPCION
			  ,@vl_fecha_doc = CONVERT(VARCHAR, FECHA_GUIA_RECEPCION, 103)
		FROM GUIA_RECEPCION
		WHERE COD_GUIA_RECEPCION = @vl_cod_doc
