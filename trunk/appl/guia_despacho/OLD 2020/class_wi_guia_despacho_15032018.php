...
class dw_item_guia_despacho extends datawindow {
	...
	function add_controls_producto_help() {		
		if (isset($this->controls['PRECIO']))
			...
			// Se guarda el old para los casos en que una validaciï¿½n necesite volver al valor OLD  
...
class wi_guia_despacho extends w_input {
				...
				if ($db->EXECUTE_SP($sp, $param)) {
					...
					$sql= "SELECT GD.COD_GUIA_DESPACHO ,
						...
						if ($cant_gd_a_hacer > 1) {
							...
							if ($i==0)
								$this->alert('Se harán '.$cant_gd_a_hacer.' guís de despacho.');
   		...
   		function envia_GD_Electronica(){
			...
			if ($db->EXECUTE_SP($sp, $param)) {
				...
				if (!$upload) {
					...
					$this->_load_record();
					$this->alert('Gestión Realizada con exito. Guia Despacho Electronica N° '.$NRO_GUIA_DESPACHO.'.');								
				}
				unlink($fname);
			}else{
				$db->ROLLBACK_TRANSACTION();
	...
	function enviar_dte(){
		...
		/*
		 * VM. SOLICITO ESTE PARCHE SOLO EN RENTAL.
		 * SII 5 ES TRASLADO INTERNO Y PASABA QUE LIBRE DTE CAMBIABA EL RECEPTOR IGUIAL AL EMISOR.
		 * 6 ES OTRSO TRASLADOS QUE NO CONSTITUYEN RENTA.
		 */
		if($IndTraslado == 5)
			$IndTraslado = 6;
		...
		$a['Encabezado']['IdDoc']['Folio']			= substr($nro_guia_despacho, 0, 10); //el folio lo da el sistema del doc.
...
class print_guia_despacho extends reporte {	
	...
	function modifica_pdf(&$pdf) {
		...
		// TAMAï¿½O DE LA CABECERA SetFont
