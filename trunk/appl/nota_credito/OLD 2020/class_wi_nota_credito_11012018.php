...
class wi_nota_credito_base extends w_cot_nv {
	...
	function load_record() {
		...
		if($COD_TIPO_NC_INTERNO_SII == 1 || $COD_TIPO_NC_INTERNO_SII == 2) //Anulaciï¿½n Factura Total
	...
	function goto_record($record) {
		if (!session::is_set("cant_nc_a_hacer")) 
			...
			if ($cant_nc_a_hacer > 1)
				$this->alert('Se harï¿½n '.$cant_nc_a_hacer.' Notas de Crï¿½dito de esta Factura.');
		}
	}
	...
	function print_record() {
		...
		if ($nro_nota_credito == -1){
			$this->redraw();		
			$this->dws['dw_nota_credito']->message("Sr(a). Usuario: Ud. no tiene documentos asignados, para imprimir la Nota de Crï¿½dito.");	
			return false;
			...
			$sql= "SELECT NC.COD_NOTA_CREDITO,
					...
					$rpt = new print_nota_credito($sql, $file_name, $labels, "Nota Crï¿½dito ".$cod_nota_credito.".pdf", 0);										
					$this->_load_record();
					$this->b_delete_visible  = false;	
					return true;
				}
	...
	// esta funcio envia mail  cuando se imprime e documento de guia despacho 
 	function f_envia_mail($estado_nota_credito){
		...
		*/
		...
		{
			$asunto = 'Impresion de Nota de Credito Nï¿½ '.$nro_nota_credito;
	        $mensaje = 'Se ha <b>IMPRESO</b> la <b>Nota de Credito Nï¿½ '.$nro_nota_credito.'</b> por el usuario <b><i>'.$remitente.'<i><b>';  
		}
	  	
	 	if($estado_nota_credito == 'ANULADA')
		{
	        $asunto = 'Anulacion de Nota de Credito Nï¿½ '.$nro_nota_credito;
	        $mensaje = 'Se ha <b>ANULADO</b> la <b>Nota de Credito Nï¿½ '.$nro_nota_credito.'</b> por el usuario <b><i>'.$remitente.'<i><b>';
		}
	...
	function Envia_DTE($name_archivo, $fname){
   	...
   	function envia_NC_Electronica(){
		...
		if ($db->EXECUTE_SP($sp, $param)) {
			...
			$CR					= $result_dte[0]['COD_TIPO_NOTA_CREDITO'];						//41 Cï¿½digo de Referencia
			$RER				= $result_dte[0]['REFERENCIA'];									//42 Razï¿½n explï¿½cita de la referencia
			...
			if (!$upload) {
				$this->_load_record();
				$this->alert('No se pudo enviar Nota Credito Electronica Nï¿½ '.$NRO_NOTA_CREDITO.', Por favor contacte a IntegraSystem.');								
			}else{
				$this->_load_record();
				$this->alert('Gestiï¿½n Realizada con exï¿½to. Nota Credito Electronica Nï¿½ '.$NRO_NOTA_CREDITO.'.');								
			}
			unlink($fname);
		}else{
			$db->ROLLBACK_TRANSACTION();
	...
	function enviar_dte(){
		...
		$a['Encabezado']['IdDoc']['Folio']			= $nro_nota_credito; //el folio lo otrorga el sistema
