...
function validate(){
	...
	if(vl_cod_tipo_nc_interno_sii == 3){//Devoluci�n Factura Parcial productos
		var aTR		 = get_TR('ITEM_NOTA_CREDITO');
		var vl_total = 0;
		var vl_nc_total = 0;
...
function change_item_nota_credito(ve_valor, ve_campo) {
	...
	if(ve_campo == 'COD_PRODUCTO' | ve_campo == 'NOM_PRODUCTO'){
		...
		if(cod_producto.value == 'T'){
			alert('No se pueden agregar T�tulos a una Nota de Credito.');
...
function entrable()
{
	if(document.getElementById('REFERENCIA_0'))
		return true;
	else
		return false;	
}
...
function dlg_print_dte_masivo(ve_cod_nc){
	var ve_cant_nc_tot = ve_cod_nc.split(',');
	var url = "dlg_print_dte_masivo.php?ve_cant_fact_tot="+ve_cant_nc_tot.length;
	$.showModalDialog({
		 url: url,
		 dialogArguments: '',
		 height: 250,
		 ...
		 onClose: function(){ 
		 	...
		 	if (returnVal == null){		
				...
				var input = document.createElement("input");
				input.setAttribute("type", "hidden");
				input.setAttribute("name", "b_print_dte_masivo_x");
				input.setAttribute("id", "b_print_dte_masivo_x");
				document.getElementById("output").appendChild(input);
