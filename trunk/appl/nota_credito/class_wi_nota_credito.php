    ...
    function envia_NC_Electronica(){
        if (!$this->lock_record())
            ...
            if ($db->EXECUTE_SP($sp, $param)) {
                ...
                $RER				= $result_dte[0]['REFERENCIA'];									//42 Razón explicita de la referencia
        ...
        if (!$this->lock_record())
            ...
            if($result[0]['NRO_NOTA_CREDITO'] <> '' && $reenviar == false)
				...
				WHERE F.COD_FACTURA = $COD_FACTURA";
                        ...
                        if($revision_estado <> 'EPR' && $revision_estado <> 'RPR' && $revision_estado <> 'RLV' && $COD_FACTURA > 59202){//ultima factura signature cod_factura 7021
                            $this->_load_record();
