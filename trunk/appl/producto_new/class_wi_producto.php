...
class dw_producto_proveedor extends dw_help_empresa{
	function dw_producto_proveedor(){
		...
		// se ajustan tamaï¿½os para la ventana de productos
...
class wi_producto_base extends w_input{
	...
	function load_record(){
		...
		$this->dws['dw_producto']->set_entrable('FACTOR_VENTA_INTERNO',false);
		$this->dws['dw_producto']->set_entrable('FACTOR_VENTA_PUBLICO',false);
	...
	function save_record($db){		
		...
		$precio_arriendo_publico	= $this->dws['dw_producto']->get_item(0, 'PRECIO_ARRIENDO_PUBLICO');
		...
		$precio_arriendo_publico	= ($precio_arriendo_publico == '') ? "null" : $precio_arriendo_publico;
		...
		/*marca en campo SISTEMA_VALIDO para que sistema es vï¿½lido el equipo
		 * solo en el insert del equipo se asignarï¿½ valor, por lo tanto no tiene update
		 */
		$sistema = $this->get_parametro(self::K_PARAM_SISTEMA);
		...
		$diametro_desague,'$maneja_inventario',$stock_critico,$tiempo_reposicion,'$precio_libre', '$es_despachable', '$sistema_valido',$potencia_kw ,$precio_arriendo_publico";
