		...
		begin
			...
			if(@vl_genera_entrada='S' and @vl_cod_bodega  is not null) begin
				...
				WHILE @@FETCH_STATUS = 0 BEGIN	
						set @vl_precio = dbo.f_bodega_pmp(@vl_cod_producto_nc, @vl_cod_bodega, getdate())
						
						  select @vl_count_producto = count(COD_PRODUCTO)
						   from PRODUCTO_COMPUESTO 
						  WHERE COD_PRODUCTO = @vl_cod_producto_nc
					...
					if(@vl_count_producto = 0 )begin	
						if (@vl_maneja_inventario= 'S') begin
							...
							AND PC.COD_PRODUCTO =  @vl_cod_producto_nc
						...
						WHILE @@FETCH_STATUS = 0 BEGIN    
							...
							set @vl_cant_producto_hijo = @vl_item_cantidad_nc * @vl_cant_producto_hijo
							set @vl_precio_hijo = dbo.f_bodega_pmp(@vl_cod_producto_hijo, @vl_cod_bodega, getdate())
