...
class wi_guia_despacho_arriendo extends wi_guia_despacho {
	const K_AUTORIZA_ENVIAR_DTE = '999005';
	const K_AUTORIZA_IMPRIMIR_DTE = '999010';
	const K_AUTORIZA_CONSULTAR_DTE = '999015';
	const K_AUTORIZA_XML_DTE = '999020';
	...
	function wi_guia_despacho_arriendo($cod_item_menu) {
		...
		$this->dws['dw_guia_despacho']->add_control($control = new drop_down_dw('COD_TIPO_GUIA_DESPACHO',$sql,150));
		...
		if ($db->EXECUTE_SP($sp, $param)) {
			...
			$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
	   		$sql_dte= "SELECT GD.COD_GUIA_DESPACHO ,
									...
									GD.GIRO,
									GD.DIG_VERIF,
									dbo.f_get_direccion('GUIA_DESPACHO', GD.COD_GUIA_DESPACHO, '[DIRECCION]') DIRECCION_FA,
									dbo.f_get_direccion('GUIA_DESPACHO', GD.COD_GUIA_DESPACHO, '[NOM_COMUNA]') NOM_COMUNA,
									dbo.f_get_direccion('GUIA_DESPACHO', GD.COD_GUIA_DESPACHO, '[NOM_CIUDAD]') NOM_CIUDAD,
									GD.TELEFONO,
									GD.FAX,
									NRO_ORDEN_COMPRA,
									NOM_PERSONA,
