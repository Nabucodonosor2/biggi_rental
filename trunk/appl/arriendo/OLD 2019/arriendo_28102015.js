...
function dlg_print() {
	...
	var returnVal = window.showModalDialog("dlg_print_arriendo.php?cod_arriendo="+cod_arriendo_value, "_blank", args);
 	if (returnVal == null)
 		return false;
	else {	
			var dato = returnVal.split('|');	  
			if (dato[1] == 'marca') {				
			args = "location:no;dialogLeft:400px;dialogTop:100px;dialogWidth:630px;dialogHeight:225px;dialogLocation:0;Toolbar:no;";
			returnVal = window.showModalDialog("dlg_print_marca.php?cod_arriendo="+cod_arriendo_value+"&cod_mod_arriendo="+dato[0], "_blank", args);
 			if (returnVal == null)
 				return false;
 			else {
...
function dlg_display_cot_arriendo(){
	...
	var returnVal = window.showModalDialog("dlg_display_cot_arr.php","_blank", args);
	if (returnVal == null)
 		return false;
	else{			
