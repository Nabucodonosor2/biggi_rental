...
class server_biggi{
	...
	function svr_print_adicional($empresa, $nro_orden_compra){
		...
		if($empresa == 'RENTAL'){
			$sql = "SELECT U.INI_USUARIO+'|'+CONVERT(VARCHAR, OC.COD_ORDEN_COMPRA)+'|'+ISNULL(CONVERT(VARCHAR, A.COD_ARRIENDO), 'NO ASOCIADO') RESULTADO
					FROM ORDEN_COMPRA OC LEFT OUTER JOIN ARRIENDO A ON A.COD_ARRIENDO = OC.COD_DOC
						,USUARIO U
	...
	function svr_print_etiqueta($nro_orden_compra){
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		$sql = "SELECT U.INI_USUARIO+'|'+CONVERT(VARCHAR, OC.COD_ORDEN_COMPRA)+'|'+ISNULL((SELECT NOM_EMPRESA FROM EMPRESA WHERE COD_EMPRESA = A.COD_EMPRESA), 'NO ASOCIADO')+'|'+ISNULL(CONVERT(VARCHAR, A.COD_ARRIENDO), 'NO ASOCIADO') RESULTADO
				FROM ORDEN_COMPRA OC LEFT OUTER JOIN ARRIENDO A ON A.COD_ARRIENDO = OC.COD_DOC
					,USUARIO U
				WHERE OC.COD_ORDEN_COMPRA = $nro_orden_compra
				AND U.COD_USUARIO = A.COD_USUARIO_VENDEDOR1";
