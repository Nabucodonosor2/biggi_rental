...
class w_cot_nv extends w_input {
	...
	function usa_precio_prod() {
	    $porc_arr		= $this->dws[$this->dw_tabla]->get_item(0, 'PORC_ARRIENDO');
		$db = new database(K_TIPO_BD, K_SERVER, K_BD, K_USER, K_PASS);
		$suma = 0;			
		for($i=0; $i<$this->dws[$this->dw_tabla_item]->row_count(); $i++){							
			$cod_producto 	= $this->dws[$this->dw_tabla_item]->get_item($i, 'COD_PRODUCTO');
			$result			= $db->build_results("select PRECIO_ARRIENDO_PUBLICO
                                                        , PRECIO_LIBRE 
                                                        ,ROUND((PRECIO_ARRIENDO_PUBLICO * $porc_arr)/ 100, 0) PRECIO_VENTA_PUBLICO_CAL
                                                    from PRODUCTO 
			...
			$this->dws[$this->dw_tabla_item]->set_item($i, 'PRECIO_ARRIENDO', $result[0]['PRECIO_VENTA_PUBLICO_CAL']);
			$cantidad 		= $this->dws[$this->dw_tabla_item]->get_item($i, 'CANTIDAD');
			$total			= $cantidad * $result[0]['PRECIO_VENTA_PUBLICO_CAL'];
