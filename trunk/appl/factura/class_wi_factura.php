...
class wi_factura_base extends w_cot_nv {
	...
	function enviar_dte($reenviar = false){
		...
		if($COD_TIPO_FACTURA == 2){
			...
			$NmbItem	= substr($Detalles[0]['NOM_PRODUCTO'], 0, 80);
			$VlrCodigo	= substr($Detalles[0]['COD_PRODUCTO'], 0, 35);
			$CANTIDAD	= 1;
			$PRECIO		= substr($Detalles[0]['PRECIO'], 0, 18);
		...
		$COD_FORMA_PAGO	= $contenido[0]['COD_FORMA_PAGO'];
	...
	function imprimir_dte($es_cedible, $desde_output=false){
		...
		if($cod_factura > 7349)
			...
			if(file_exists("../../../../PDF/PDFCOMERCIALBIGGI/$year/33_$nro_factura.pdf"))
				print " <script>window.open('../../../../PDF/PDFCOMERCIALBIGGI/$year/33_$nro_factura.pdf')</script>";
			else
