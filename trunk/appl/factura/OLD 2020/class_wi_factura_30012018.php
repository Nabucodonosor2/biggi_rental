...
class dw_factura extends dw_help_empresa {
	...
	function dw_factura() {
		$sql = "SELECT	F.COD_FACTURA,
					...
					,null TABLE_ITEM_FACTURA
					,null TABLE_ITEM_CONTRATO
					,REFERENCIA_HEM
					,REFERENCIA_HES
					,ORIGEN_FACTURA
		...
		$this->add_control(new static_text('SUBTOTAL'));
...
class dw_bitacora_factura extends datawindow {
	function dw_bitacora_factura() {
		...
		// mandatory
		$this->set_mandatory('COD_ACCION_COBRANZA', 'Acciï¿½n de cobranza');
...
class wi_factura_base extends w_cot_nv {
	...
	function load_record() {
		...
		if ($COD_ESTADO_DOC_SII == self::K_ESTADO_SII_EMITIDA) {
			 ...
			 * Y se agrega un perfil de autorizaciï¿½n de quienes pueden anular FA
			 */
			$this->dws['dw_factura']->set_item(0, 'VISIBLE_DTE', 'none');
			...
			if($this->tiene_privilegio_opcion(self::K_PUEDE_ANULAR_FACTURA)){
				...
				if (($date2[1] == $date1[1] && $date2[2] == $date1[2]) ||	// mismo mes y mismo aï¿½o
	...
	function print_record() {
		...
		if ($nro_factura == -1){
			...
			if ($db->EXECUTE_SP($sp, $param)) {
				...
				if ($cod_estado_doc != $estado_sii_impresa){//es la 1era vez que se imprime la Factura					
					...
					$this->redraw();
						if($porc_iva ==0)
							$this->dws['dw_factura']->message("Sr(a). Usuario: Se imprimirï¿½ la Factura Exenta Nï¿½".$nro_factura);
						else
							$this->dws['dw_factura']->message("Sr(a). Usuario: Se imprimirï¿½ la Factura Nï¿½".$nro_factura);
		...
		//		return false;
			...
			// establecer una conexiï¿½n bï¿½sica
			$conn_id = ftp_connect($K_DIRECCION_FTP);
			if ($conn_id===false)
				return false; 
			
			// iniciar una sesiï¿½n con nombre de usuario y contraseï¿½a
		...
		function envia_FA_electronica(){
			...
			if ($db->EXECUTE_SP($sp, $param)) {
				...
				if (!$upload) {
					...
					if ($PORC_IVA == 0){
						$this->_load_record();
						$this->alert('Gestión Realizada con exíto. Factura Exenta Electronica N° '.$NRO_FACTURA.'.');
					}else{
						$this->_load_record();
						$this->alert('Gestión Realizada con exíto. Factura Electronica N° '.$NRO_FACTURA.'.');
					}								
				}
				unlink($fname);
			}else{
		...
		function f_envia_mail($estado_factura){
			...
			*/
			...
			{
				$asunto = 'Impresion de Factura Nï¿½ '.$nro_factura;
		        $mensaje = 'Se ha <b>IMPRESO</b> la <b>FACTURA Nï¿½ '.$nro_factura.'</b> por el usuario <b><i>'.$remitente.'<i><b>';  
			 }
		  	
		 	if($estado_factura == 'ANULADA')
			{
		        $asunto = 'Anulacion de la Factura Nï¿½ '.$nro_factura;
		        $mensaje = 'Se ha <b>ANULADO</b> la <b>FACTURA Nï¿½ '.$nro_factura.'</b> por el usuario <b><i>'.$remitente.'<i><b>';
			}
	...
	function enviar_dte(){
		...
		if($COD_TIPO_FACTURA == 2){
			...
			$NmbItem	= $Detalles[0]['NOM_PRODUCTO'];
			$VlrCodigo	= $Detalles[0]['COD_PRODUCTO'];
			$CANTIDAD	= 1;
			$PRECIO		= $Detalles[0]['PRECIO'];
			$MONTO_TOTAL= $Detalles[0]['TOTAL_FA'];
			
			if($cod_tipo_dte == 34){
				$ad['Detalle'][0]["IndExe"]= 1;	
			}
			
			$ad['Detalle'][0]["NmbItem"]= utf8_encode(trim($NmbItem));
			$ad['Detalle'][0]["CdgItem"]= $VlrCodigo;
			$ad['Detalle'][0]["QtyItem"]= $CANTIDAD;
			$ad['Detalle'][0]["PrcItem"]= $PRECIO;
		...
		$a['Encabezado']['IdDoc']['Folio']			= $nro_factura; //el folio lo da el sistema de facturacion.
		...
		//Guarda el response de la funciï¿½n emitir_dte.
		$db->BEGIN_TRANSACTION();
		$sp = 'spu_factura';
		...
		if($rep_response[1] <> ''){
			...
			if (($nro_fa_dte <> '') && ($EnvioDTExml <> '')&& ($track_id <> '')){
	            ...
	            if ($db->EXECUTE_SP($sp, $param)) {
		            ...
		            if(session::is_set('ENVIAR_DTE_MASIVO')){
						session::un_set('ENVIAR_DTE_MASIVO');
					}else{
