...
function ajax_valida_nro_doc(){
	var vl_nro_doc = document.getElementById('NRO_NCPROV_0').value;
	if(vl_nro_doc != ""){
		var ajax = nuevoAjax();
		ajax.open("GET", "ajax_valida_nro_doc.php?nro_doc="+vl_nro_doc, false);
		ajax.send(null);
		var resp = ajax.responseText;
		
		if(parseInt(resp) > 0){
			alert('El Número Doc. '+vl_nro_doc+' ya esta emitida o aprobada.');
			document.getElementById('NRO_NCPROV_0').value = "";
		}
	}
}