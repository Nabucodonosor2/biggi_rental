...
class wi_arriendo extends w_cot_nv {
	...
	function creada_desde_cot_arr($cod_cot_arriendo){
		...
		if($day_registro_cot > $validez_oferta_cot){
			$this->alert("La Cotización de arriendo N° ".$cod_cot_arriendo.", se encuentra fuera del periodo de validez de oferta (".$validez_oferta_cot." días.) Se usarón precios actualizados.");
			$no_valido = true;
		...
		for($i=0 ; $i < count($result_item); $i++){
			...
			if($no_valido){
				$sql_precio="SELECT ROUND((PRECIO_ARRIENDO_PUBLICO * $porc_arr)/ 100, 0) PRECIO_VENTA_PUBLICO_CAL
				...
				$precio_tot = $result_precio[0]['PRECIO_ARRIENDO_PUBLICO'] * $result_item[$i]['CANTIDAD'];
				
				$this->dws['dw_item_arriendo']->set_item($i, 'PRECIO_VENTA',	$result_precio[0]['PRECIO_ARRIENDO_PUBLICO']);
