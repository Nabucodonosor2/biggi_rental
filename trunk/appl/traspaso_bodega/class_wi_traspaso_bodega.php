...
class dw_item_traspaso_bodega extends datawindow {
	function dw_item_traspaso_bodega() {		
		...
		$control->set_onChange("stock_bodega_origen(this);");
		...
		$this->controls['COD_PRODUCTO']->size= 27;
		$this->controls['NOM_PRODUCTO']->size= 98;
		
		$js = $this->controls['COD_PRODUCTO']->get_onChange();
		$js =$js."stock_bodega_origen(this);";
...
class dw_traspaso_bodega extends datawindow {
	...
	function dw_traspaso_bodega() {
		...
		$control->set_onChange("change_bodega();");
...
class wi_traspaso_bodega extends w_input {
	...
	function print_record() {
		...
		$sql= "SELECT ITEM
					  ...
					  ,NOM_PRODUCTO
					  ,CANTIDAD
					  ,CONVERT(VARCHAR, FECHA_TRASPASO_BODEGA, 103) FECHA_TRASPASO_BODEGA
					  ...
					  ,REFERENCIA
			   FROM TRASPASO_BODEGA TB
				   ,ITEM_TRASPASO_BODEGA ITB
				   ,USUARIO U
			   WHERE TB.COD_TRASPASO_BODEGA = $cod_traspaso_bodega
			   AND TB.COD_TRASPASO_BODEGA = ITB.COD_TRASPASO_BODEGA
			   AND TB.COD_USUARIO = U.COD_USUARIO";

		$labels = array();
		$labels['strCOD_TRASPASO_BODEGA'] = $cod_traspaso_bodega;				
		$file_name = $this->find_file('traspaso_bodega', 'traspaso_bodega.xml');				

		$rpt = new reporte($sql, $file_name, $labels, "Traspaso Bodega ".$cod_traspaso_bodega.".pdf", 1);
		$this->_load_record();
		return true;
	}
}
