...
class wi_gasto_fijo_base extends w_input {
	...
	function new_record() {
		...
		$this->dws['dw_orden_compra']->set_item(0, 'COD_CUENTA_CORRIENTE', 3);
		if (K_CLIENTE=='RENTAL')
			$this->dws['dw_orden_compra']->set_item(0, 'NRO_CUENTA_CORRIENTE', 3);
	...
	function habilitar(&$temp, $habilita){
		...
		if($COD_ESTADO_ORDEN_COMPRA == 1){// emitida
			$priv = $this->get_privilegio_opcion_usuario('994030', $this->cod_usuario);
			if($priv == 'E')
