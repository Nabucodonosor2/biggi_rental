...
function add_documento_cheque(ve_cambio_cant_ch){
	...
	for (i=0; i<aTR.length; i++){
		...
		document.getElementById('MONTO_DOC_'+ vl_row).innerHTML = number_format(vl_total_con_iva, 0, '.', '.');
...
function actualiza_fecha() {
	...
	for (i=0; i<aTR.length; i++){
		var vl_row = get_num_rec_field(aTR[i].id);
		document.getElementById('FECHA_DOC_'+ vl_row).innerHTML = result[i]['FECHA_DOCUMENTO_PAGO'];
