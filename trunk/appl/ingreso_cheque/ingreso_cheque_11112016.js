function validate(){
	...
	if (aTR.length==0) {
		alert('Debe tener al menos un item como documento de pago antes de grabar.');
...
//elimina todos los registros.
	...
	if(result != ''){	
		for (i=0; i < result.length; i++) {
			...
			if(number_format(result[i]['TOTAL_ACTUAL'], 0, '.', '.') == 0){
				result[i]['TOTAL_ACTUAL'] = result[i]['TOTAL_CON_IVA'];
				document.getElementById('ID_TR_COLOR_' + vl_row).style.backgroundColor = '#A9D0F5';
			}
			
			document.getElementById('TOTAL_ACTUAL_' + vl_row).innerHTML 			= number_format(result[i]['TOTAL_ACTUAL'], 0, '.', '.');			
...
function suma_arriendo_total(){
	...
	for (i=0; i<aTR.length; i++){
		...
		if (check == true){
			...
			var total_actual = to_num(get_value('TOTAL_ACTUAL_' + rec_tr)); 
			sum_total_con_iva = parseInt(sum_total_con_iva)+ parseInt(total_con_iva);
			sum_total_actual = parseInt(sum_total_actual)+ parseInt(total_actual);
		}
	}
	
	document.getElementById('SUM_ARRIENDO_TOTAL_0').innerHTML = number_format(sum_total_con_iva, 0, '.', '.');
	document.getElementById('SUM_ARRIENDO_ACTUAL_0').innerHTML = number_format(sum_total_actual, 0, '.', '.');
...
function add_documento_cheque(ve_cambio_cant_ch){
	...
	// actualiza monto	
	var vl_sum_arriendo_actual = to_num(get_value('SUM_ARRIENDO_ACTUAL_0')); 
	var sum_monto_doc = 0;
	var aTR = get_TR('CHEQUE');
	for (i=0; i<aTR.length; i++){
		var vl_row = get_num_rec_field(aTR[i].id);
		document.getElementById('MONTO_DOC_'+ vl_row).value = number_format(vl_sum_arriendo_actual, 0, '.', '.');
		document.getElementById('MONTO_DOC_H_'+ vl_row).value = number_format(vl_sum_arriendo_actual, 0, '.', '.');
		sum_monto_doc = parseInt(sum_monto_doc) + parseInt(vl_sum_arriendo_actual);
	}
...
function actualiza_monto(ve_campo){
	...
	//MH: No mostrar estos alert hasta que se regularicen los casos antiguos
