...
class wo_cheque extends w_output_biggi{
	...
	function wo_cheque() {
		...
		$sql = "SELECT E.COD_EMPRESA
					  ,NOM_EMPRESA
				FROM EMPRESA E
				WHERE E.COD_EMPRESA in (SELECT DC.COD_EMPRESA
									   FROM DEPOSITO_CHEQUE DC)";

		$this->add_header(new header_drop_down('NOM_EMPRESA', 'E.COD_EMPRESA', 'Razon Social', $sql));
	...
	function redraw_item(&$temp, $ind, $record) {
		...
		if ($liberado=='S' && $depositado=='S')
			$temp->setVar("wo_registro.TR_COLOR", "#A7F4F8");
		else
			$temp->setVar("wo_registro.TR_COLOR", "");
	}
