...
class wi_bodega extends w_input {
	...
	function wi_bodega($cod_item_menu) {
		...
		$sql = "SELECT COD_TIPO_BODEGA
					  ,NOM_TIPO_BODEGA
				FROM TIPO_BODEGA";
		$this->dws['dw_bodega']->add_control(new drop_down_dw('COD_TIPO_BODEGA', $sql, 120));
		$this->dws['dw_bodega']->set_entrable('COD_TIPO_BODEGA', false);
