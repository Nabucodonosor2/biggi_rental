...
class dw_factura extends dw_help_empresa {
	...
	function dw_factura() {
		...
		$this->add_control(new edit_check_box('NO_TIENE_OC','S','N','Sin OC'));
			...
			if (($nro_fa_dte <> '') && ($EnvioDTExml <> '')&& ($track_id <> '')){
	            ...
	            if ($db->EXECUTE_SP($sp, $param)) {
		            ...
		            if(session::is_set('ENVIAR_DTE_MASIVO')){
						...
						print " <script>window.open('../common_appl/print_dte.php?cod_documento=$cod_factura&DTE_ORIGEN=33')</script>";
					}
