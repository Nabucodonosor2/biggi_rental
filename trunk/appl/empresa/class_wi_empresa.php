...
class dw_bitacora_empresa extends datawindow {
	function dw_bitacora_empresa() {		
		...
		// asigna los mandatorys	
		$this->set_mandatory('NOM_BITACORA_EMPRESA', 'Observacion de la Bitácora');
	}
	function insert_row($row = -1) {
		$row = parent::insert_row($row);
...
class wi_empresa_base extends w_input {
	function wi_empresa_base($cod_item_menu) {
		...
		$this->dws['dw_empresa']->set_mandatory('NOM_EMPRESA', 'Razonn Social');		
		...
		*/
		...
		$this->dws['dw_factura']->set_entrable_dw(false);	/// no funcionï¿½ dejar no entrable la dw.  IS 25/02/09
	...
	function save_record($db) {
		...
		//se valida en la funciï¿½n validate que se ingresen los demï¿½s campos mandatory
		// EL CAMPO PORC_DSCTO_CORPORATIVO FUE ELIMINADO DE LA BD	
