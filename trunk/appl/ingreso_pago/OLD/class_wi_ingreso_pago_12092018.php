...
class wi_ingreso_pago_base extends w_input {
	...
	function new_record() {
		...
		$this->dws['dw_ingreso_pago']->set_item(0, 'COD_PROYECTO_INGRESO', 1);
	}
	...
	function envio_mail(){
		...
		$sql = "SELECT CONVERT(VARCHAR, NRO_FACTURA) COD_DOCUMENTO
		...
		if(count($result) > 1){
			$asunto	= "'Pago F/VARIAS $".$sum_monto_asignado." CL. $nom_empresa / SISTEMA WEB RENTAL'";			
		}else if(count($result) == 1){
			$cod_documento = $result[0]['COD_DOCUMENTO'];
			$asunto = "'Pago F/$cod_documento $".$sum_monto_asignado." CL. $nom_empresa / SISTEMA WEB RENTAL'";
		}
